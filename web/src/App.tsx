...
function getStatusColor(status: SystemStatus | undefined): string {
...
async function fetchStatus(): Promise<SystemStatus> {
  ...
  const data = (await res.json()) as { data: SystemStatus }
  return data.data
}

function App() {
  const [currentPage, setCurrentPage] = useState<Page>(PAGES.CHAT)
  ...
  // Sync sessions from React Query to Zustand sessionStore
  useEffect(() => {
    setSessions(
      sessions.map((s) => ({
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <Suspense fallback={<div className="p-8 text-center text-muted">Loading...</div>}>
          {currentPage === PAGES.KB ? (
            ...
            <ChatView systemStatus={systemStatus} />
