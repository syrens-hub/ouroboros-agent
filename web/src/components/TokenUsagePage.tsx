...
async function fetchTokenUsage(sessionId: string | undefined, granularity: string, days: number): Promise<TokenUsageRow[]> {
  const qs = new URLSearchParams()
  if (sessionId) qs.set('sessionId', sessionId)
  qs.set('granularity', granularity)
  qs.set('days', String(days))
  const res = await apiFetch(`/api/token-usage?${qs.toString()}`)
  const json = (await res.json()) as { data?: TokenUsageRow[] }
  return json.data || []
}
...
function SimpleAreaChart({ data, height = 160 }: SimpleAreaChartProps) {
...
export function TokenUsagePage({ sessions = [] }: TokenUsagePageProps) {
