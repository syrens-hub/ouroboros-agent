...
vi.mock('../api', () => ({
  ...
  wsUrl: (sessionId?: string) => ({
    url: `ws://test/ws?sessionId=${sessionId || ''}`,
    protocols: undefined,
  }),
}))
...
describe('ChatView', () => {
  ...
  beforeEach(() => {
    ...
    mockFetch.mockImplementation((path: string) => {
      if (path === '/api/sessions') {
        return Promise.resolve({ json: async () => ({ data: sessions }) })
      }
  ...
  afterEach(() => {
    globalThis.URL.createObjectURL = globalThis.__originalCreateObjectURL as typeof globalThis.URL.createObjectURL
    globalThis.URL.revokeObjectURL = globalThis.__originalRevokeObjectURL as typeof globalThis.URL.revokeObjectURL
    vi.unstubAllGlobals()
    delete (Element.prototype as unknown as Record<string, unknown>).scrollIntoView
  })

  it('renders session list and empty state', () => {
    render(<ChatView systemStatus={{}} />, { wrapper: Wrapper })
  ...
  it('sends a chat message via websocket', async () => {
    ...
    ws.onopen?.()
    expect(ws.send).toHaveBeenCalledWith(JSON.stringify({ type: 'chat', message: 'hello bot' }))
  })
  ...
  it('shows confirm modal and handles confirm', async () => {
    ...
    act(() => {
      ws.onmessage?.({ data: JSON.stringify({ event: 'confirm_request', data: { toolName: 'write_file', input: { path: 'x' }, timeoutMs: 60000 } }) })
    })
    await waitFor(() => {
    ...
    expect(ws.send).toHaveBeenCalledWith(JSON.stringify({ type: 'confirm', allowed: true }))
  })
  ...
  describe('multimodal', () => {
    const recInstances: MockSpeechRecognition[] = []
    beforeEach(() => {
    ...
    it('toggles voice input with SpeechRecognition', async () => {
      ...
      await act(async () => {
        const resultItem = [{ transcript: 'hello voice' }] as unknown as SpeechRecognitionResult
        ;(resultItem as unknown as { isFinal: boolean }).isFinal = true
        recInstance.onresult!({ resultIndex: 0, results: [resultItem] } as unknown as SpeechRecognitionEvent)
      })
    ...
    it('escapes script tags in user message', async () => {
      ...
      const { container } = render(<ChatView systemStatus={{}} />, { wrapper: Wrapper })
      await userEvent.click(screen.getByText('Session One'))
