...
interface CrewRunBody {
  task: string
  roles: unknown[]
  process: string
}

interface SopRunBody {
  definition: unknown
...
async function runCrew(body: CrewRunBody): Promise<CrewResponse> {
  const res = await apiFetch('/api/crew/run', {
    method: 'POST',
    body: JSON.stringify(body),
  })
  return (await res.json()) as CrewResponse
}

async function fetchSopTemplates(): Promise<unknown> {
  const res = await apiFetch('/api/sop/templates')
  return res.json()
}

async function runSop(body: SopRunBody): Promise<SopResponse> {
  const res = await apiFetch('/api/sop/run', {
    method: 'POST',
    body: JSON.stringify(body),
  })
  return (await res.json()) as SopResponse
}
...
export function WorkflowStudio() {
  ...
  const templates = Array.isArray((templatesData as { data?: SopTemplate[] } | undefined)?.data)
    ? (templatesData as { data: SopTemplate[] }).data
    : []
  ...
  const handleRunCrew = () => {
    ...
    try {
      parsedRoles = JSON.parse(roles) as unknown[]
    } catch {
      parsedRoles = []
    }
  ...
  const handleRunSop = () => {
    const definition = sopMode === 'template' && activeTemplate
      ? activeTemplate
      : (() => {
          try { return JSON.parse(customSop) as unknown } catch { return undefined }
        })()
