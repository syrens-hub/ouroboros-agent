...
async function fetchPatterns(): Promise<unknown> {
  const res = await apiFetch('/api/learning/patterns')
  return res.json()
}

async function fetchConfig(sessionId: string): Promise<unknown> {
  const res = await apiFetch(`/api/learning/config/${sessionId}`)
  return res.json()
}

export function LearningInsights({ sessionId = 'demo-session' }: LearningInsightsProps) {
  ...
  const rawPatterns =
    (patternsData as { data?: { patterns?: LearningPattern[] } | LearningPattern[] } | undefined)?.data?.patterns ??
    (patternsData as { data?: LearningPattern[] } | undefined)?.data ??
    []
  const patterns = Array.isArray(rawPatterns) ? rawPatterns.slice(0, 3) : []
  const config = (configData as { data?: LearningConfig } | undefined)?.data || {}
