...
async function fetchScreenshots(): Promise<unknown> {
  const res = await apiFetch('/api/gallery/screenshots')
  return res.json()
}

async function drawCanvas(body: DrawCanvasBody): Promise<DrawResponse> {
...
export function Gallery() {
  ...
  const screenshots = (screenshotsData as { data?: Screenshot[] } | undefined)?.data || []

  const addCommand = (type: CanvasCommand['type']) => {
    const base: CanvasCommand = { type }
    if (type === 'rect') {
      base.x = 50
      base.y = 50
      base.width = 100
