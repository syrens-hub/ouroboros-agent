...
async function approveEvolution({ approvalId, versionId, changedFiles, approved }) {
  const res = await apiFetch('/api/evolution/approve', {
    method: 'POST',
    body: JSON.stringify({ approvalId, versionId, changedFiles, approved }),
  })
  return res.json()
}

async function rollbackEvolution({ versionId }) {
              ...
              <div key={v.id} className="flex items-center justify-between border border-border/50 rounded-lg p-3 text-xs">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <StatusBadge status={v.appliedAt ? 'applied' : v.approvalStatus} />
                  </div>
