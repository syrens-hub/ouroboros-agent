...
import { Shield, GitBranch, CheckCircle, XCircle, Clock, AlertTriangle } from 'lucide-react'
    ...
    <div className="bg-card border border-border rounded-xl p-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        ...
        <div className="bg-secondary/30 rounded-lg p-3">
          ...
          <div className="text-white font-medium flex items-center gap-2">
            {eventBus.pendingDeadLetters > 0 ? <AlertTriangle className="w-3.5 h-3.5 text-warn" /> : <CheckCircle className="w-3.5 h-3.5 text-ok" />}
            {eventBus.pendingDeadLetters}
          </div>
        </div>
        <div className="bg-secondary/30 rounded-lg p-3">
          <div className="text-xs text-muted">版本数</div>
          <div className="text-white font-medium">{evolutionVersions.totalVersions}</div>
        </div>
        <div className="bg-secondary/30 rounded-lg p-3">
          <div className="text-xs text-muted">待审批</div>
          <div className="text-white font-medium flex items-center gap-2">
            {approvals.pendingCount > 0 ? <Clock className="w-3.5 h-3.5 text-warn" /> : <CheckCircle className="w-3.5 h-3.5 text-ok" />}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        ...
        <div className="flex items-center justify-between text-xs py-1 border-b border-border/50">
          ...
          <StatusBadge ok={!safety.frozen} label={safety.frozen ? `冻结 ${safety.freezeRemainingHours.toFixed(1)}h` : '未冻结'} />
        </div>
        <div className="flex items-center justify-between text-xs py-1 border-b border-border/50">
          <span className="text-muted">Budget</span>
          <StatusBadge ok={safety.budget.withinBudget} label={safety.budget.withinBudget ? '正常' : '超支'} />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-xs py-1 border-b border-border/50">
        ...
        <span className="text-white font-mono">{evolutionMetrics.totalEvolutions}</span>
      </div>
      <div className="flex items-center justify-between text-xs py-1 border-b border-border/50">
        <span className="text-muted">成功率</span>
        <span className="text-white font-mono">{evolutionMetrics.successRate.toFixed(1)}%</span>
      </div>
      <div className="flex items-center justify-between text-xs py-1">
        <span className="text-muted">回滚率</span>
        <span className={evolutionMetrics.rollbackRate > 10 ? 'text-danger font-mono' : 'text-white font-mono'}>{evolutionMetrics.rollbackRate.toFixed(1)}%</span>
      </div>
