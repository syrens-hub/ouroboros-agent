...
async function fetchEvolutionMetrics() {
  try {
...
export function EvolutionTimeline() {
  ...
  const isExpanded = (id) => expandedAll || expandedId === id
      ...
      {metrics && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="bg-secondary/20 border border-border rounded-lg p-3">
            ...
            <div className="text-lg font-semibold text-text-strong">${metrics.totalCostUsd.toFixed(4)}</div>
          </div>
          <div className="bg-secondary/20 border border-border rounded-lg p-3">
            <div className="flex items-center gap-1.5 text-xs text-muted mb-1">
      ...
      <div className="relative pl-4">
        ...
        <div className="space-y-4">
          {filtered.map((item) => {
            ...
            const meta = item.metadata
            const id = meta?.id || commit.shortHash
            ...
            return (
              <div key={id} className="relative">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-mono text-accent">{commit.shortHash}</span>
                      <span className="text-sm text-text-strong truncate max-w-[16rem] sm:max-w-xs">
                        {commit.message}
                      </span>
                      {meta && meta.riskLevel > 0 && (
                        <span className={`text-[10px] px-1.5 py-0.5 rounded border ${riskBadge(meta.riskLevel)}`}>
                          R{meta.riskLevel}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-xs text-muted">{commit.date?.slice(0, 10)}</span>
                      {expanded ? (
                  ...
                  {expanded && meta && (
                    <div className="mt-3 pt-3 border-t border-border/60 space-y-2">
                      ...
                      {commit.stats && (
                        <div className="text-xs text-muted">
                          Diff: <span className="text-ok">+{commit.stats.insertions}</span>{' '}
                          <span className="text-danger">-{commit.stats.deletions}</span>{' '}
