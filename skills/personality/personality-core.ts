...
import * as anchorStore from "./anchor-store.ts";
...
export class PersonalityEvolution {
  ...
  addAnchorMemory(params: Omit<AnchorMemory, "id" | "createdAt" | "reinforcementCount" | "lastAccessedAt">): void {
    const anchor = anchorStore.insertAnchor(this.sessionId, params);
  ...
  reinforceAnchor(anchorId: string): void {
    const anchor = anchorStore.reinforceAnchor(this.sessionId, anchorId);
    if (!anchor) return;
    this.reinforceFromAnchor(anchor);
  }
  ...
  getAnchorMemories(category?: AnchorMemory["category"]): AnchorMemory[] {
    return anchorStore.getAnchors(this.sessionId, category);
  }
  ...
  getRelevantAnchors(query: string, limit = 5): AnchorMemory[] {
    return anchorStore.getRelevantAnchors(this.sessionId, query, limit);
  }
  ...
  reset(): void {
    ...
    anchorStore.deleteSessionAnchors(this.sessionId);
  }
